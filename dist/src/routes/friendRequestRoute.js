...
route.post('/AcceptRequest', async (req, res) => {
    ...
    try {
        ...
        const updateFriendReq = await FriendRequestModel.findOneAndUpdate({ senderId, receiverId, status: "pending" }, { status: "accepted" }, { new: true });
        if (!updateFriendReq) {
...
route.post('/rejectRequest', async (req, res) => {
    ...
    try {
        ...
        const updateFriendReq = await FriendRequestModel.findOneAndUpdate({ senderId, receiverId, status: "pending" }, { status: "rejected" }, { new: true });
        if (!updateFriendReq) {
            return res.status(404).json({ message: "Friend request not found" });
        }
        io.to(senderId).emit('FriendRequest', {
...
const getFriendsId = async (ids) => {
    ...
    return friends.map(f => idArray.includes(f.senderId.toString())
        ? f.receiverId.toString()
        : f.senderId.toString());
};
route.get('/getRequest/:userId', async (req, res) => {
    const { userId } = req.params;
    try {
        ...
        for (const sender of pendingUsers) {
            ...
            const mutualFriendsCount = senderFriends.filter(id => userFriendSet.has(id)).length;
            payload.push({
